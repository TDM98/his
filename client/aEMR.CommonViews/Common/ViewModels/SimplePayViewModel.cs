 ...
 * 20200814 #011 TTM:   BM 0041458: [SimplePay] Lỗi hiển thị thông tin tiền bệnh nhân chi trả.
 * 20200906 #012 TNHX:   BM: [QMS] Tự động gọi STT sau khi trả tiền
 * 20201211 #013 TNHX:   BM: Tự động gửi work list qua PAC
 * 20210717 #014 TNHX:  Thêm chỗ nhập mã code thanh toán online
 * 20220804 #015 QTD:   Cấu hình cấp số QMS từ kế toán thu
 * 20221019 #016 QTD:   Thêm cấu hình in phiếu chỉ định khi bấm Lưu và trả tiền
 * 20221027 #017 QTD:   Thêm đánh dấu dịch vụ ưu tiên QMS
 * 20230306 #018 QTD:   Thêm thông báo đăng ký đã quyết toán
 */
namespace aEMR.Common.ViewModels
{
    public class PayForRegistrationDetails
    {
        ...
        /// </summary>
        public PatientTransaction CurPtTransaction { get; set; }
        /// <summary>
        /// Thông tin tính tiền do server trả về sau khi thực hiện xong.
        /// </summary>
        public PatientTransactionPayment PatientPayment { get; }

        /// <summary>
        /// Thông tin tính tiền do server trả về sau khi thực hiện xong.
        /// </summary>
        public List<PaymentAndReceipt> PatientPaymentList { get; }
        public PatientRegistration CurRegistration { get; }

        public PatientTransactionPayment _paymentDetails;

        /// <summary>
        /// Danh sách những chi tiết đăng ký muốn trả tiền.
        /// </summary>
        public List<PatientRegistrationDetail> _paidRegDetailsList;

        /// <summary>
        /// Danh sách những PCL Request muốn trả tiền.
        /// </summary>
        public List<PatientPCLRequest> _paidPclRequestList;
        public List<OutwardDrugInvoice> _paidDrugInvoiceList;
        public List<OutwardDrugClinicDeptInvoice> _paidMedItemList;
        public List<OutwardDrugClinicDeptInvoice> _paidChemicalItemList;
        public IList<InPatientBillingInvoice> _billingInvoiceList;
        public int? _Apply15HIPercent;
        public bool _checkBeforePay;
        public long? ConfirmHIStaffID = null;
        public string OutputBalanceServicesXML;
        public bool IsReported = false;
        public bool IsUpdateHisID = false;
        public PayForRegistrationDetails(PatientRegistration regInfo, PatientTransactionPayment paymentDetails,
            ...
            , List<InPatientBillingInvoice> aBillingInvoiceCollection = null)
        {
            ...
            _billingInvoiceList = aBillingInvoiceCollection;
        }
    }

    [Export(typeof(ISimplePay)), PartCreationPolicy(CreationPolicy.NonShared)]
    public class SimplePayViewModel : ViewModelBase, ISimplePay
    {
        ...
        public SimplePayViewModel(IWindsorContainer container, INavigationService navigationService, ISalePosCaching salePosCaching)
        {
        ...
        public bool IsRefundBilling { get; set; } = false;
        ...
        private void ResetPatientPayment()
        {
            ...
            if (V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.QUAY_XAC_NHAN)
            {
                ...
                try
                {
                    if (!IsUpdateHisID && !IsOutPtTreatmentPrescription)
                    {
                ...
                && Registration.ConfirmHIStaffID.GetValueOrDefault(0) == 0)
            {
                ...
                if (!IsRefundFromPharmacy && V_TradingPlaces != (long)AllLookupValues.V_TradingPlaces.DANG_KY)
                {
            ...
            else if (V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.QUAY_XAC_NHAN && !IsOutPtTreatmentPrescription)
            {
            ...
            else if (V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.QUAY_XAC_NHAN && IsOutPtTreatmentPrescription)
            {
                ...
                if (DrugPayAmount < 0)
                {
                    DrugInvoices = Registration.DrugInvoices.Where(x => x.V_OutDrugInvStatus == (long)AllLookupValues.V_OutDrugInvStatus.CANCELED && x.PaidTime != null && x.PaidTime != DateTime.MinValue && x.RefundTime == null).ToObservableCollection();
                }
                CurrentPayment.PayAmount = DrugPayAmount;
            }
        ...
        private decimal CalcPatientPayment()
        {
            ...
            if (RegistrationDetails != null)
            {
                foreach (var item in RegistrationDetails)
                {
                    ...
                    {
                        //▼===== 20200729 TTM: Nếu dịch vụ chưa trả tiền thì tính như bình thường. 
                        //                     Còn các dịch vụ đã trả tiền rồi mà thay đổi thông tin 
                        //                     thì kiểm tra số tiền đã trả và số tiền thay đổi để tính toán lại cho chính xác simplepay

                        if (item.PaidTime == null)
                        {
                            payment += CalPaymentForMedRegItem(item);
                        }
                        else if (item.PaidTime != null && item.RecordState == RecordState.MODIFIED)
                        {
                            payment += (item.TotalPatientPayment + item.DiscountAmt) - Registration.PatientTransaction.PatientTransactionDetails.Where(x => x.PtRegDetailID == item.PtRegDetailID).Sum(y => y.PatientPayment);
                        }
                        //▲=====
            ...
            if (DrugInvoices != null)
            {
                ...
                if (DrugInvoices.Where(x => x.OutwardDrugs != null && x.PaidTime == null).SelectMany(x => x.OutwardDrugs).Any(x => x.PaidTime == null)
                    //&& Registration.PtInsuranceBenefit.GetValueOrDefault(0) == 0
                    && V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.NHA_THUOC)
                {
                    payment = 0;
                }
                            ...
                            || invoice.V_OutDrugInvStatus == (long)AllLookupValues.V_OutDrugInvStatus.DRUGCOLLECTED)
                        {
                            if (invoice.OutwardDrugs != null && invoice.PaidTime == null)
                            {
                                ...
                                else
                                {
                                    ...
                                    //▼===== 20200120 TTM: Chỉ làm tròn trước khi show popup trả tiền. Không làm tròn ở chi tiết
                                    payment += MathExt.Round(TotalInvoicePrice - TotalHIPayment, Converters.MidpointRounding.AwayFromZero);
                                    //payment += TotalInvoicePrice - TotalHIPayment;
                                    //▲===== 20200120 TTM: 
                                }
                            }
                        }
            ...
            //▼===== #011:  Dời vị trí của billinginvoice ngoại trú lên trên Drug vì nếu như bệnh nhân có bill ngoại trú chưa tiền thì 
            //              bất cứ chỗ nào đưa simplepay lên đều log tiền bill ngoại trú (Kể cả nhà thuốc) => Lỗi.
            //if (Registration.InPatientBillingInvoices != null && Registration.InPatientBillingInvoices.Count > 0
            //    && Registration.InPatientBillingInvoices.Any(x => x.V_InPatientBillingInvStatus != AllLookupValues.V_InPatientBillingInvStatus.NGUNG_TRA_TIEN_LAI))
            //{
            //    foreach (var aItem in Registration.InPatientBillingInvoices.Where(x => x.V_InPatientBillingInvStatus != AllLookupValues.V_InPatientBillingInvStatus.NGUNG_TRA_TIEN_LAI).ToList())
        ...
        private IEnumerator<IResult> LoaddAllSelectionRefValues()
        {
                ...
                && CurrentPayment != null)
            {
              ...
              && (((Registration.AllSavePCLRequestDetails != null && Registration.AllSavePCLRequestDetails.Any(x => x.PaidTime == null))
                  ...
                  && (Registration.AllSavePCLRequestDetails == null || !Registration.AllSavePCLRequestDetails.Any(x => x.PaidTime == null && x.HosClientContractID == 0))
                  && (Registration.AllSaveRegistrationDetails == null || !Registration.AllSaveRegistrationDetails.Any(x => x.PaidTime == null && x.HosClientContractID == 0))))
            {
                CurrentPayment.PtPmtAccID = 4;
                IsEnablePatientPmtAcc = false;
            }
                ...
                && Registration.Appointment.ClientContract.IsPayAddingMoreSvs)
            {
                ...
                && (((Registration.AllCanceledPCLRequestDetails != null && Registration.AllCanceledPCLRequestDetails.Any(x => x.RefundTime == null))
                    ...
                    && (Registration.AllCanceledPCLRequestDetails == null || !Registration.AllCanceledPCLRequestDetails.Any(x => x.RefundTime == null && x.HosClientContractID == 0))
                    && (RegistrationDetails == null || !RegistrationDetails.Any(x => x.V_ExamRegStatus == (long)AllLookupValues.ExamRegStatus.NGUNG_TRA_TIEN_LAI && x.RefundTime == null && x.HosClientContractID == 0))))
            {
                CurrentPayment.PtPmtAccID = 4;
                IsEnablePatientPmtAcc = false;
            }
            ...
            var t = new Thread(() =>
            {
                try
                {
                    using (var serviceFactory = new BillingPaymentWcfServiceLibClient())
                    {
                        ...
                        var newServiceList = regPaymtDet.CurRegistration.PatientRegistrationDetails.Where(item => item.RecordState == RecordState.ADDED || item.RecordState == RecordState.DETACHED
                                                                    || ((item.RecordState == RecordState.UNCHANGED || item.RecordState == RecordState.MODIFIED) && item.PaidTime == null)).ToList();
                        ...
                        contract.BeginSaveThenPayForServicesAndPCLReqs(Globals.LoggedUserAccount.StaffID.GetValueOrDefault()
                            ...
                            , CurrentPayment.TranPaymtNote, CurrentPayment.PaymentMode.LookupID
                            //▲====: #014
                            , regPaymtDet.CurRegistration.V_ReceiveMethod
                            , Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
        ...
        private IEnumerator<IResult> DoPayForRegistration()
        {
            ...
            if (exError == null)
            {
                ...
                {
                    bool isPaid = true;
                    bool hasPaymentOrder = false;
                    List<string> resultCreateOrders = GlobalsNAV.CreateOrders(isPaid, hasPaymentOrder, Registration.Patient, ref paidRegDetailsList, 
                        ref paidPclRequestList, false, null, null, true);
                    if(resultCreateOrders != null && resultCreateOrders.Count > 0)
                    {
                        var orderTask = new GenericCoRoutineTask(UpdateOrderNumber, resultCreateOrders);
                        yield return orderTask;
                    }
                    UpdateDoneOrderNumberStatus();
                }
                //▲===== #015
                ...
                if (V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.DANG_KY && Registration.TicketIssue != null)
                {
                    if (Globals.ServerConfigSection.CommonItems.UseQMSSystem
                        && IsUpdateStatus
                        && Registration.TicketIssue != null && Globals.ServerConfigSection.CommonItems.UpdateTicketStatusAfterRegister)
                    {
                ...
                //▲===== #010
                //▼===== #012
                if (Globals.ServerConfigSection.CommonItems.UseQMSSystem && V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.DANG_KY)
                {
                    Globals.EventAggregator.Publish(new CallNextTicketQMSEvent());
                }
                //▲===== #012
                ...
                //▼====: #013
                // 20201209 TNHX: Trả tiền
                if (Globals.ServerConfigSection.Pcls.AutoCreatePACWorklist && paidPclRequestList != null)
                {
                    ObservableCollection<PatientPCLRequest> aPatientPCLRequestList = paidPclRequestList.Where(x => x.V_PCLMainCategory == (long)AllLookupValues.V_PCLMainCategory.Imaging && !x.IsCancelTransferredToRIS).ToObservableCollection();
                    if (aPatientPCLRequestList.Count > 0)
                    {
                        foreach (PatientPCLRequest temp in aPatientPCLRequestList)
                        {
                            temp.Patient = Registration.Patient;
        ...
        private void UpdateDoneOrderNumberStatus()
        {
            if (!GlobalsNAV.IsQMSEnable())
            {
                return;
            }
            OrderDTO orderDTO = GlobalsNAV.UpdateOrder(Registration.Patient, OrderDTO.DONE_STATUS);

            if (null == orderDTO)
            {
                MessageBox.Show("Không thể cập nhật trạng thái của STT. Vui lòng chọn lại quầy/ phòng phù hợp!");
            }
            else if (orderDTO.patientCode == Registration.Patient.PatientCode
                && 0 != Registration.Patient.OrderNumber
                && Registration.Patient.OrderNumber != orderDTO.orderNumber)
            {
                MessageBox.Show("Thông tin bệnh nhân và STT không khớp.");
            }
        }
        ...
        private IEnumerator<IResult> DoPayForRegisNew_CombiningTwoStoreProcs()
        {
            ...
            if (exError == null)
            {
                ...
                if (Globals.ServerConfigSection.CommonItems.IsEnableCreateOrderFromAccountant)
                {
                    ...
                    bool hasPaymentOrder = false;
                    //▼===== #017
                    List<PatientRegistrationDetail> paidRegDetailsListToQMS = RegistrationDetails.Where(x => paidRegDetailsList.Any(y => x.RefMedicalServiceItem.MedServiceID == y.MedServiceID && (x.DeptLocation != null && x.DeptLocation.DeptLocationID == y.DeptLocID))).ToList();
                    List<string> resultCreateOrders = GlobalsNAV.CreateOrders(isPaid, hasPaymentOrder, Registration.Patient, ref paidRegDetailsListToQMS,
                        ref paidPclRequestList, false, null, null, true);
                ...
                // 20211215 Không in biên lai đối với BN khám sức khỏe
                if (savedRegisReturn.PatientClassification != null && savedRegisReturn.PatientClassification.PatientClassID != (long)ePatientClassification.CompanyHealthRecord)
                {
                ...
                //▼===== 20200719 TTM: Bổ sung code cập nhật tình trạng thẻ khi đăng ký bằng cách sử dụng lưu và trả tiền.
                if (Globals.ServerConfigSection.CommonItems.UseQMSSystem
                    && V_TradingPlaces == (long)AllLookupValues.V_TradingPlaces.DANG_KY
                    && savedRegisReturn.TicketIssue != null
                    && IsUpdateStatus
                    && savedRegisReturn.TicketIssue != null && Globals.ServerConfigSection.CommonItems.UpdateTicketStatusAfterRegister)
                {
                    UpdateTicketStatusAfterRegister(savedRegisReturn.TicketIssue.TicketNumberText, savedRegisReturn.TicketIssue.TicketGetTime);
                }
                //▲===== 
                //▼===== 20200719
                //▼===== #012
                ...
                if (!IsConfirmHIView)
                {
                    ...
                    // 20200908 TNHX: Thêm điều kiện để in phiếu chỉ định đối với BN khám sức khỏe
                    if (Globals.ServerConfigSection.CommonItems.PrintingPhieuChiDinhForService && (savedRegisReturn.PatientClassification != null && savedRegisReturn.PatientClassification.PatientClassID != (long)ePatientClassification.CompanyHealthRecord))
                    //if (Globals.ServerConfigSection.CommonItems.PrintingPhieuChiDinhForService)
                    {
                        Globals.EventAggregator.Publish(new PhieuChiDinhForRegistrationCompleted() { Registration = savedRegisReturn, 
                            RegDetailsList = Globals.ServerConfigSection.CommonItems.IsEnablePrintReceiptAndRequest ? paidRegDetailsList : new List<PatientRegistrationDetail>(), PCLRequestList = paidPclRequestList ?? new List<PatientPCLRequest>() });
                    }
                    else if (savedRegisReturn.PatientClassification != null && savedRegisReturn.PatientClassification.PatientClassID == (long)ePatientClassification.CompanyHealthRecord)
                    {
                        List<PatientRegistrationDetail> PatientRegistrationDetailsTemp = new List<PatientRegistrationDetail>();
                        foreach(var temp in RegistrationDetails.ToObservableCollection())
                        {
                            PatientRegistrationDetail curDetail = paidRegDetailsList.Where(x => x.MedServiceID == temp.MedServiceID).FirstOrDefault();
                            if (curDetail != null && curDetail.MedServiceID != 0)
                            {
                                PatientRegistrationDetailsTemp.Add(curDetail);
                            }
                        }
                        List<PatientPCLRequest> PatientPCLRequestTemp = new List<PatientPCLRequest>();
                        foreach (var temp in PclRequests.ToObservableCollection())
                        {
                            if (temp.PatientPCLRequestIndicators != null && temp.PatientPCLRequestIndicators.Count() > 0)
                            {
                                foreach (var tempPCL in temp.PatientPCLRequestIndicators)
                                {
                                    PatientPCLRequest curPCL = paidPclRequestList.Where(x => x.PCLExamTypeID == tempPCL.PCLExamType.PCLExamTypeID).FirstOrDefault();
                                    if (curPCL != null && curPCL.PCLExamTypeID != 0)
                                    {
                                        PatientPCLRequestTemp.Add(curPCL);
                                    }
                                }
                            }
                        }
                        Globals.EventAggregator.Publish(new PhieuChiDinhForRegistrationCompleted() { Registration = savedRegisReturn, RegDetailsList = PatientRegistrationDetailsTemp, PCLRequestList = PatientPCLRequestTemp });
                    }
                }
                ...
                if (Globals.ServerConfigSection.Pcls.AutoCreatePACWorklist && paidPclRequestList != null)
                {
                    ObservableCollection<PatientPCLRequest> aPatientPCLRequestList = paidPclRequestList.Where(x => x.V_PCLMainCategory == (long)AllLookupValues.V_PCLMainCategory.Imaging && x.PaidTime != null && !x.IsCancelTransferredToRIS).ToObservableCollection();
                    if (aPatientPCLRequestList.Count > 0)
                    {
                        foreach (PatientPCLRequest temp in aPatientPCLRequestList)
                        {
        ...
        public bool IsUpdateStatus
        {
            ...
            set
            {
                ...
                NotifyOfPropertyChange(() => IsUpdateStatus);
            }
        }
        ...
        {
            get { return _IsOutPtTreatmentPrescription; }
            set
            {
        ...
        //▼====: #014
        private bool _IsShowPaymentOnlineCode = false;
        public bool IsShowPaymentOnlineCode
        {
            get
            {
                return _IsShowPaymentOnlineCode;
            }
            set
            {
                _IsShowPaymentOnlineCode = value;
                NotifyOfPropertyChange(() => IsShowPaymentOnlineCode);
            }
        }

        public void cboPaymentMode_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            Lookup SelectedItem = (sender as System.Windows.Controls.ComboBox).SelectedItem as Lookup;
            if (SelectedItem == null)
            {
                return;
            }
            if (SelectedItem.LookupID == (long)AllLookupValues.PaymentMode.THANH_TOAN_ONLINE)
            {
                IsShowPaymentOnlineCode = true;
                return;
            }
        }
        //▲====: #014
        private static void UpdateOrderNumber(GenericCoRoutineTask genTask, object _orderNumLst)
        {
            List<string> orderNumLst = (List<string>)_orderNumLst;
            if (null == orderNumLst || orderNumLst.Count < 1)
            {
                MessageBox.Show(GlobalsNAV.CANNOT_CREATE_ORDER_MSG, eHCMSResources.T0074_G1_I, MessageBoxButton.OK);
                return;
            }
                        ...
                        mContract.BeginUpdateOrderNumberRegistration(orderNumLst,
                            Globals.DispatchCallback((asyncResult) =>
                            {
                                try
                                {
                                    ...
                                    if (!IsOK)
                                    {
                                        MessageBox.Show(GlobalsNAV.CANNOT_CREATE_ORDER_MSG);
                                    }
                                }
                                catch (Exception ex)
                                {
